...
use super::ghcb::{GhcbHandle, GHCB};
use super::racycell::*;
...
fn lazy_ghcb_ext() -> Mutex<&'static mut SnpGuestDriver> {
    static DRIVER: Lazy<RacyCell<SnpGuestDriver>> = Lazy::new(lazy_refcell);
...
impl SnpGuestDriver {
    ...
    fn dec_payload(
        ...
        let asssoc_data =
            unsafe { core::slice::from_raw_parts(&response.hdr.algo as *const _ as *const u8, 48) };
    ...
    pub fn get_attestation(
        ...
        if tee::get_tee_type() != crate::CCMode::SevSnp {
            return Ok([0, 0]);
        }
