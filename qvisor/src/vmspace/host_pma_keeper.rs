...
impl HostPMAKeeper {
    ...
    pub fn InitHugePages(&self) {
        ...
        info!("VMM: PMA - Init with HugePages - Len is {:x}G",
            self.ranges.lock().range.Len() / MemoryDef::ONE_GB - 2);
    ...
    pub fn Unmap(&self, r: &Range) -> Result<()> {
        ...
        debug!("VMM: Unmap file length:{:#0x}B, guest mapped:{:#0x}", r.Len(), r.Start());
        #[cfg(target_arch = "aarch64")] {
            if is_hw_tee() {
